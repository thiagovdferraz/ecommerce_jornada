...
# Configurações do DataLake (carregadas de .env)
...
# Configurações do PostgreSQL (Supabase) carregadas de .env
...
# Estamos usando o schema `datalake.ecommerce` para todas as tabelas
...
for tabela, df in dataframes.items():
    ...
    df.to_sql(
        tabela,  # Nome da tabela no banco (sem schema)
        engine,  # Engine de conexão
        schema="datalake_ecommerce",  # Schema onde gravar as tabelas
